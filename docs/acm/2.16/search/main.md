...
2.  List the resources in the `data.AllowedResources` and the
    `data.DeniedResources` fields within the config map.

    1.  Collect services and pods from all `apiGroups`, while also
        collecting all resources from the `admission.k8s.io` and
        `authentication.k8s.io` `apiGroups`.

    2.  Configure your config map to prevent the central collection of
        secrets from all `apiGroups`, while preventing the collection of
        `policies`, `iampolicies`, and `certificatepolicies` from the
        `admission.k8s.io` `apiGroup` .
    ...
    **Note:** If you do not provide a config map, all resources are
    collected by default. When you specify `AllowedResources`, all other
    resources are automatically excluded. If you list resources in both
    the `AllowedResources` and `DeniedResources`, resources are excluded
    from the collection.

3.  Add the `backup` label to include the `search-collector-config`
    config map to the cluster backups. Run the following command:
...
1.  Modify the default values in the `search-v2-operator` custom
    ...
    2.  Configure a persistent volume claim for clusters that are in
        production. If the empty directory size is limited, store the
        PostgreSQL data on a persistent volume claim to improve search
        performance.

        **Notes:**

        - During installation, the default configuration for the Red Hat
          Advanced Cluster Management PostgreSQL database uses an
          `emptyDir` volume for data storage.

        - If a persistent volume claim is missing, the
          `SearchPVCNotPresent` alert is visible in the **Status** pane.

        - If a persistent volume claim for search is missing and
          critical conditions exist, you receive the
          `SearchPVCNotPresentCritical` Prometheus alert.
...
2.  Optimize cost by tuning the pod memory or CPU requirements, replica
    ...
    1.  Update the `deployment` section of the `search-v2-operator`
        ...
        `search-v2-operator`, which you can update individually.
...
For help to configure persistent storage, see Configuring persistent
storage in the OpenShift Container Platform documentation
...
2.  Append the resource requirements as annotations. When you define
    both `annotations` and `resourceRequirements`, the `annotations`
    value overrides the `resourceRequirements` value. View the following
    example:
    ...
    **Notes:**
    ...
    - You can discover all resources defined in your managed cluster by
      using the API Explorer in the console. Alternatively, you can
...
3.  Create an `AddonDeploymentConfig` resource and update the limit in
    the `resourceRequirements` field for the `klusterlet-addon-search`
    deployment . Your `AddonDeploymentConfig` resource might resemble
...
4.  Add the `backup` label to include the `AddOnDeploymentConfig`
    resource to the cluster backups. Run the following command:
...
5.  Apply the `search-addon-resources` `AddOnDeploymentConfig` file to
    your klusterlet add-on by running the following command:

    ``` bash
    oc apply -f search-addon-resources.yaml
    ```

6.  Update the `klusterlet-addon-search` deployment to use the new
    `AddonDeploymentConfig` resource. Run the following command:
