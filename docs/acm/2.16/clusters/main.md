...
Learn about multicluster engine for Kubernetes operator 2.11 new
features and enhancements, support, deprecations, removals, and fixed
issues for errata releases.

multicluster engine operator version 2.11 is released with Red Hat
Advanced Cluster Management version 2.16 for Cluster lifecycle
...
- You can now create an Microsoft Azure Red Hat OpenShift hosted control
  plane managed cluster by using Cluster API Provider for Azure to
  reduce costs and setup time. See Preparing to create Microsoft Azure
  Red Hat OpenShift hosted control plane clusters by using Cluster API
  Provider for Azure to learn more.
...
`ClusterPool` custom resource definition, Hive might delete and
re-create all unclaimed clusters. This is expected behavior and ensures
that unclaimed clusters reflect the latest configuration defined in the
...
When a change occurs in the cluster because of the `Placement` resource,
cluster redistribution occurs across multiple `PlacementDecisions`
resources. During the sequential update, a cluster is temporarily
missing from all `PlacementDecisions` resources.

Controllers that monitor `PlacementDecisions` resources identify this
temporary deletion as cluster removal. This sequence leads to
unnecessary resource deletion and recreation. For example, the
`addon-management-controller` deletes and re-creates
`ManagedClusterAddOns`, which cause workload disruption.

ACM-27799

##### HyperShift operator and Cluster API components conflict

Red Hat Advanced Cluster Management and multicluster engine operator
enable the `hypershift` and `hypershift-local-hosting` components by
default.

To manage Red Hat OpenShift Service on AWS (classic) clusters by using
the `cluster-api` and `cluster-api-provider-aws` components, you must
disable the `hypershift` and `hypershift-local-hosting` components to
prevent API conflicts.

When you enable and then disable the `cluster-api` and
`cluster-api-provider-aws` components, custom resource definitions of a
higher version might remain in the cluster.

You must manually remove the custom resource definitions before you
re-enable the `hypershift-local-hosting` component. Use the following
...
##### HyperShift installation fails on Red Hat OpenShift Service on AWS (classic) due to admission webhook error

When you install multicluster engine for Kubernetes operator 2.10 on Red
Hat OpenShift Service on AWS (classic), the HyperShift installation job
might fail repeatedly with an admission webhook error similar to the
following message:
`namespace-validation.managed.openshift.io denied the request`. See the
following explanations for the error:
...
- The Red Hat OpenShift Service on AWS (classic) managed admission
  webhooks prevent users from modifying protected labels on namespaces,
  including the `cluster-monitoring` label.
...
- In managed environments, such as Red Hat OpenShift Service on AWS
  (classic), the service provider typically handles platform-level
  monitoring. Therefore, this monitoring is not required and conflicts
  with namespace protection policies.
...
2.  Copy the `bootArtifacts` and add them to the proxy by running the
    following commands:
...
- Product or category: ManagedClusterAddon - Affected item: The
  ...
  spec. - Version: 2.10 - Recommended action: Use the
  AddonDeploymentConfig field. - More details and links: None

- Product or category: Documentation for APIs - Affected item: The
  multicluster engine operator API documentation - Version: 2.8 -
  Recommended action: View current and supported APIs from the console
  or the terminal instead of the documentation - More details and links:
  None
...
3.  Create a `ConfigMap` file in the `multicluster-engine` namespace
    ...
    multicluster engine operator, for example, `2.11`:
...
- A hub cluster cannot manage a different hub cluster. A hub cluster is
  set up to automatically import and manage itself as a `local-cluster`.
  You do not have to manually import a hub cluster to manage.

- The namespace for the imported cluster must match the name of the
  cluster.
...
#### Running an Ansible Automation Platform job to update the control plane and specific node pools

To run an Ansible Automation Platform job that updates the control plane
and specific node pools, edit the `nodePoolNames` field in your
`ClusterCurator` resource of the hosted cluster that you want to update.
See the following example:
...
#### Running an Ansible Automation Platform job to update the control plane

To run an Ansible Automation Platform job that updates the control
plane, set the `upgradeType` field to `ControlPlane` in your
`ClusterCurator` resource of the hosted cluster that you want to update.
See the following example:
...
#### Running an Ansible Automation Platform job to update the node pools

To run an Ansible Automation Platform job that updates all node pools
associated with your hosted cluster, set the `upgradeType` field to
`NodePools` in your `ClusterCurator` resource that you want to update.
See the following example:
...
#### Running an Ansible Automation Platform job to update specific node pools

To run an Ansible Automation Platform job that updates only specific
node pools, set the `upgradeType` field to `NodePools` in your
`ClusterCurator` resource and list the node pools that you want to
update in the `nodePoolNames` field. See the following example:
...
#### Running an Ansible Automation Platform job to update the cluster channel

To run an Ansible Automation Platform job that updates the cluster
channel without changing the version, add the cluster channel to the
`channel` field. See the following example:
...
#### Running an Ansible Automation Platform job to update the cluster version and channel

To run an Ansible Automation Platform job that updates the cluster
version and channel, add the cluster version to the channel to the
`desiredUpdate` field and add the cluster channel to the `channel`
...
3.  Set up the Red Hat OpenShift Service on AWS (classic)
    `DiscoveredCluster` resource for import as a managed cluster. The
    Red Hat OpenShift Service on AWS (classic) import flow that is
    initiated uses the service-account credentials authenticate and
    import the cluster.
...
- You must set up the Red Hat OpenShift Service on AWS (classic)
  command-line interface. See Getting started with the Red Hat OpenShift
  Service on AWS (classic) CLI documentation.

#### Importing discovered Red Hat OpenShift Service on AWS (classic) and hosted control plane clusters automatically

The following procedure is an example of how to import your discovered
Red Hat OpenShift Service on AWS (classic) and hosted control planes
clusters automatically by using the `Discovery-Operator`. You can import
from the console or from the command-line interface.
...
- Import from the command-line interface.
  ...
  3.  To get a description of your Red Hat OpenShift Service on AWS
      (classic) cluster ID, run the following command from the Red Hat
      OpenShift Service on AWS (classic) command-line interface:
...
a deployment model for Red Hat OpenShift Service on AWS (classic) where
the cluster control plane is hosted and managed in a Red Hat Amazon Web
...
- The AWS Red Hat OpenShift Service on AWS (classic) managed policies
  are attached to your AWS user permissions. See AWS managed policies
  for ROSA in the AWS documentation to learn more.
...
- You need the ROSA CLI (`rosa`). See ROSA CLI in the Red Hat OpenShift
  Service on AWS (classic) documentation to learn more.
...
### Preparing to create Microsoft Azure Red Hat OpenShift hosted control plane clusters by using Cluster API Provider for Azure

<div wrapper="1" role="_abstract">

Before creating an Azure Red Hat OpenShift hosted control plane managed
cluster by using Cluster API Provider for Azure, you must complete the
...
- You need a valid Microsoft Azure subscription with permissions to
  create Azure Red Hat OpenShift hosted control plane resources.

- You need the OpenShift CLI. See OpenShift CLI (oc) in the OpenShift
  Container Platform documentation to learn more.

- You need the `jq` tool. See ./jq to learn more.

- You need the Azure Service Operator v2. See Azure Service Operator v2
  to learn more.
...
1.  Check if the `hypershift` and `hypershift-local-hosting` components
    are disabled. Run the following command:
...
2.  If the components are enabled, disable both components.

    1.  Run the following command to disable the `hypershift` component:
    ...
    2.  Run the following command to disable the
        `hypershift-local-hosting` component:
...
3.  Verify that the components are disabled. Run the following command:
...
4.  Enable the Cluster API component. Run the following command:
...
5.  Verify that the Cluster API component is enabled. Run the following
    command:
...
6.  Enable the Cluster API Provider for Azure component. Run the
    following command:
...
7.  Verify that the Cluster API Provider for Azure component is enabled.
    Run the following command:
...
8.  Verify that the operators are ready. Run the following command:

    ``` bash
    oc get deployment -n multicluster-engine | grep -E "(capi|capz|azureserviceoperator)"
    ```

    Make sure the output shows `1` in the `AVAILABLE` column for each
...
9.  Configure your Microsoft Azure credentials.

    1.  Create a service principal with permissions to manage Azure Red
        Hat OpenShift hosted control plane resources. Run the following
        command. Replace `<subscription_id>` with your subscription ID:
        ...
        Save the following information from the output for the next
        step:
    ...
    2.  Create a namespace-scoped `Secret` resource with your Microsoft
        Azure credentials for Azure Service Operator. Use the following
        YAML sample:
        ...
        - Replace \<tenant_id\> with the `tenant` value from the
          previous step.

        - Replace \<client_id\> with the `appId` value from the previous
          step.

        - Replace \<client_secret\> with the `password` value from the
          previous step.

        **Note:** The namespace-scoped `Secret` resource overrides
        global settings for all resources in that namespace. The
        namespace-scoped `Secret` resource is used for objects in the
        default namespace. It is not aligned with provisioning Azure Red
        Hat OpenShift hosted control plane clusters by using Cluster API
        Provider for Azure. Provisioning uses the `aro-clusters`
        namespace.
...
- To learn more about Azure Red Hat OpenShift, see the Azure Red Hat
  OpenShift documentation.

### Creating Microsoft Azure Red Hat OpenShift hosted control plane clusters by using Cluster API Provider for Azure

<div wrapper="1" role="_abstract">

Create an Azure Red Hat OpenShift hosted control plane managed cluster
by using Cluster API Provider for Azure to reduce costs and setup time
by swapping dedicated main virtual machines for a managed, containerized
control plane. Use a GitOps workflow to provision and scale clusters by
using the same declarative manifests that you use for your applications.
...
- You must complete the preparation procedure. See Preparing to create
  Azure Red Hat OpenShift hosted control plane clusters by using Cluster
  API Provider for Azure to learn more.

- You need the `clusterctl` tool. See Overview of clusterctl to learn
  more.

**Procedure**

1.  Create a cluster namespace. Run the following command:

    ``` bash
    oc create namespace aro-clusters
    ```

2.  Create a YAML file named `aro-hcp-cluster.yaml` to define your
    cluster resources. Replace `<subscription_id>` with your
    subscription ID. Your resource might resemble the following example:
...
3.  Create an `AROCluster` resource and define your infrastructure
    resources in `spec.resources`. Replace `<subscription_id>` with your
    subscription ID. Your resource might resemble the following example:
...
4.  Create an `AROControlPlane` resource and define your control plane
    resources in `spec.resources` field. Your resource might resemble
    the following example:
...
5.  Create a `MachinePool` resource and define your node pool resources
    in `spec.resources` field. Your resource might resemble the
    following example:
...
6.  Create an `AROMachinePool` resource and define your node pool
    resources in `spec.resources` field. Your resource might resemble
    the following example:
...
7.  Run the following command to apply the file:

    ``` bash
    oc apply -f aro-hcp-cluster.yaml
    ```

    **Note:** It might take 15 to 20 minutes for the managed cluster to
    provision.

**Verification**

1.  Check the status of the Cluster API resources by running the
    following command:
...
2.  Check the status of the Azure Service Operator resources by running
    the following command:
...
3.  Check the Cluster API Provider for Azure controller logs by running
    the following command:
...
4.  Check your managed cluster status.

    1.  Check the Cluster API status. Run the following command:

        ``` bash
        oc get cluster aro-hcp-cluster -n aro-clusters
        ```

    2.  **Optional:** Check the detailed cluster status and conditions.
        Run the following command:
        ...
        If your cluster is ready, the output reseembles the following
        example:
...
5.  Verify your infrastructure resources.

    1.  Check the `AROCluster` status. Run the following command:

        ``` bash
        oc get arocluster aro-hcp-cluster -n aro-clusters
        ```

    2.  Check the `AROControlPlane` status. Run the following command:

        ``` bash
        oc get arocontrolplane aro-hcp-cluster -n aro-clusters
        ```

    3.  Check the `AROMachinePool` status. Run the following command:

    ``` bash
    oc get aromachinepool -n aro-clusters
    ```

6.  Verify your Azure Service Operator resources.

    1.  Check the hosted control plane cluster status in Microsoft
    ...
    2.  Check the node pool status. Run the following command:

    ``` bash
    oc get HcpOpenShiftClustersNodePool -n aro-clusters
    ```

7.  Get your managed cluster `kubeconfig` file. The `kubeconfig` is
    automatically created when the managed cluster is ready. Run the
    following command:
...
8.  Test the connection to your managed cluster. Run the following
    command:
...
9.  Get the console URL from the control plane status. Run the following
    command:
...
### Deleting Azure Red Hat OpenShift hosted control plane clusters by using Cluster API Provider for Azure

<div wrapper="1" role="_abstract">

Deleting an Azure Red Hat OpenShift hosted control plane managed cluster
deprovisions the cluster and deletes the associated resources. The
process takes 10 to 15 minutes to complete.
...
- You have an Azure Red Hat OpenShift hosted control plane managed
  cluster.

**Procedure**

1.  Delete your cluster by running the following command:
...
1.  Check if the Cluster API resources are removed. Run the following
    command:
...
2.  Check if the Azure Service Operator resources are removed. Run the
    following command:
...
3.  Check if the Microsoft Azure resources are removed. Run the
    following commands:
...
### Troubleshooting managed clusters *Unknown* on Red Hat OpenShift Service on AWS (classic) with hosted control planes cluster

The status of all managed clusters on a Red Hat OpenShift Service on AWS
(classic) hosted clusters suddenly becomes `Unknown`.

#### Symptom: All managed clusters are in *Unknown* status on Red Hat OpenShift Service on AWS (classic) with hosted control planes cluster
...
#### Resolving the problem: All managed clusters are in *Unknown* status on Red Hat OpenShift Service on AWS (classic) with hosted control planes cluster
