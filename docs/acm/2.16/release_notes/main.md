...
Learn about Red Hat Advanced Cluster Management 2.16 new features and
...
Red Hat Advanced Cluster Management for Kubernetes 2.16 provides
...
Red Hat Advanced Cluster Management version 2.16 is released with the
multicluster engine operator version 2.11 for Cluster lifecycle
...
Red Hat Advanced Cluster Management is available on the AWS Marketplace
with either on-demand or annual pricing and a simplified billing option
for running Red Hat Advanced Cluster Management on Red Hat OpenShift
Service on AWS (classic) clusters. Red Hat Advanced Cluster Management
on the AWS Marketplace offers billing consistency, flexible resource
...
- Use the *Attach* feature with Red Hat OpenShift Lightspeed, a
  generative AI-powered virtual assistant for Red Hat OpenShift
  Container Platform, to add managed cluster resource details as context
  for questions. See the Red Hat OpenShift Lightspeed installation
  documentation to learn how to install Lightspeed. After installing,
  click **Infrastructure** \> **Clusters** \> **Cluster details** on any
  managed cluster page to access the assistant. The *Attach* feature
  automatically adds the relevant cluster YAML file details from the
  `ManagedCluster` and `ManagedClusterInfo` resource to the agent
  conversation.
...
- The console now supports hosted control plane upgrade paths and
  displays a channel modal to manage channels for hosted clusters, with
  available channels for upgrade in the `ManagedClusterInfo` resource.

- Manage large search datasets by configuring pagination with the
  `limit`, `offset`, and `orderBy` parameters in the search API. For
  more information, see Search query API.

- You receive a `SearchPVCNotPresentCritical` Prometheus alert when a
  persistent volume claim for search is missing and critical conditions
  exist on your system. For more information, see Customizing the Search
  service.
...
- The `RightSizingRecommendation` feature to optimize workloads is now
  generally available. After you enable the observability service,
  namespace right-sizing and virtualization right-sizing are enabled by
  default. To learn more, see RightSizingRecommendation guides.

- You can now configure resource requirements for observability using
  the `AddonDeploymentConfig` resource when observability is enabled
  through the multicluster observability add-on. For more information,
  see Enabling the multicluster observability add-on and Setting up the
  AddOnDeploymentConfig to configure klusterlet add-ons. See
  Observability service to learn more.
...
- Fine-grained role-based access control (RBAC) for virtualization is
  now generally available. Fine-grained RBAC provides administrators
  with the precision to manage permissions across managed clusters with
  greater security and flexibility. Learn more in the Fine-grained
  role-based access control for virtual machines introduction.

- The Red Hat Advanced Cluster Management Fleet Management interface now
  includes specialized roles designed specifically for managing virtual
  machines across clusters, which extend standard virtualization
  capabilities, offering specific access levels for fleet-wide
  operations. See Fine-grained RBAC roles and descriptions for more
  information about the roles for virtualization.

- With the new `MultiClusterRoleAssignment` custom resource, you can
  manage role-based access control (RBAC) for virtual machines. As an
  administrator, use the declarative method to centralize permission
  logic on the hub cluster for propagation with managed clusters. See
  Fine-grained role-based access control MulticlusterRoleAssignment to
  learn more. See examples in MulticlusterRoleAssignment examples for
  fine-grained role-based access control.

- You can now edit the `HyperConverged` resource to improve flexibility
  when your infrastructure requirements change and fine-tune your
  deployment. See Migrating virtual machines between clusters to learn
  more.

- New roles for virtualization are added to Red Hat Advanced Cluster
  Management for cross cluster migration and access to the Fleet
  Management console. See the following roles for virtualization:

- acm-vm-extended:view: Provides read-only access to Kubernetes
  resources.

- acm-vm-extended:admin: Provides administrative access to Kubernetes
  resources.

- acm-vm-fleet:view: Provides the minimum permissions required to use
  the Fleet Virtualization console.

- acm-vm-fleet:admin: Provides the permissions that are required to use
  the Fleet Virtualization console and perform cluster-to-cluster live
  migration.

- acm-vm-cluster-migration:view: Provides the permissions that are
  required for migration readiness checks. See Migration of virtual
  machines between clusters for more information about roles for
  migration.

See Virtualization for more information.
...
##### Passive sync restore does not recover after transient *BackupStorageLocation* failures

When you configure a `Restore` resource in passive sync mode by setting
`syncRestoreWithNewBackups` to `true`, and the `BackupStorageLocation`
becomes temporarily unavailable, the Velero restore operation fails with
a `FailedValidation` status.

After you restore the `BackupStorageLocation` and create successsful
backups, the Red Hat Advanced Cluster Management `Restore` resource
remains in the `Error` phase. This happens because failed Velero restore
objects persist in the cluster, which prevents the controller from
attempting new restores.

The issue causes the following conditions:

- The secondary hub cluster appears unhealthy.

- Automated disaster recovery scripts or tools identify the cluster as
  not ready for switchover.

- The `Restore` operation does not automatically resume sync operations
  after you resolve the `BackupStorageLocation` issue.

To resume the backup sync operation, manually delete the failed Velero
restore objects. Run the following command. Replace
`<failed_restore_name>` with the name of the failed Velero restore
object:
...
After the failed objects are deleted, the `Restore` controller performs
...
Service on AWS (classic) `cluster-ID` from the terminal. You need the
Red Hat OpenShift Service on AWS (classic) command line interface. See
Getting started with the Red Hat OpenShift Service on AWS (classic) CLI
documentation.
...
`fine-grained-rbac` feature. Complete the following steps:

1.  Disable `fine-grained-rbac` in the `MultiClusterHub` resource.
...
The *Console* and *Topology* for *Application* changes for the 2.16.
...
When you upgrade to Red Hat Advanced Cluster Management 2.15 and later,
alert forwarding fails if you disabled automatic configuration by
setting `mco-disable-alerting: true` in the `MultiClusterObservability`
custom resource, and if you manually configured alert forwarding.

Alert forwarding fails after you upgrade because the name of the
`observability-alertmanager-accessor` and `hub-alertmanager-router-ca`
secrets were changed to include a postfix of the hub cluster ID.

The hub cluster ID is obtained from the `uuid` field in the metadata of
the `ClusterVersion` custom resource on the hub cluster. The ID, with
dashes removed and trimmed to the first 19 characters, is appended to
the `alertmanager` secrets.
...
#### Configuration policy listed compliant when namespace is stuck in *Terminating* state
...
detailed steps on how to set this limit, see Deploying and Managing AMQ
Streams on OpenShift.
...
- Product or category: Documentation for APIs - Affected item: The Red
  Hat Advanced Cluster Management API documentation - Version: 2.13 -
  Recommended action: View current and supported APIs from the console
  or the terminal instead of the documentation. - More details and
  links: None

- Product or category: Application management - Affected item:
  Subscription - Version: 2.13 - Recommended action: Use Red Hat
  Advanced Cluster Management with OpenShift GitOps instead. - More
  details and links: The deprecation is extended for five releases
  before removal. See GitOps overview for updated function.
...
- Product or category: Red Hat Edge Manager with Red Hat Advanced
  Cluster Management - Affected item: edge-manager-preview component in
  the MultiClusterHub resource and integrated documentation - Version:
  2.15 - Recommended action: See Red Hat Edge Manager 1.0 - More details
  and links: See Red Hat Edge Manager 1.0

- Product or category: Backup and restore - Affected item: The
  backup-restore-auto-import policy - Version: 2.16 - Recommended
  action: Use the backup-restore-enabled policy instead. - More details
  and links: The backup-restore-auto-import policy templates are merged
  into the existing backup-restore-enabled policy. The
  hub-backup-policy-set.yaml file is updated to reference only the
  backup-restore-enabled policy.

- Product or category: Console - Affected item: Infrastructure \>
  Virtual machines console page - Version: 2.15 - Recommended action: On
  Red Hat OpenShift Container Platform version 4.20, install Red Hat
  OpenShift Virtualization 4.20 to work with virtual machines across
  your fleet from the Fleet Virtualization perspective. Alternatively,
  use Search to find and take actions on virtual machines. - More
  details and links: None
...
- Product or category: Search service - Affected item: Enabling virtual
  machine actions (Technology Preview) - Version: 2.14 - Recommended
  action: Use fine-grained role-base access control instead. - More
  details and links: See Role-based access control.
...
cryptographic libraries submitted for validation, see FIPS - Federal
