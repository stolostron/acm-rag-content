...
Before you perform migration operations or access the Fleet
Virtualization console, ensure that the following requirements are met:

</div>

1.  Enable fine-grained RBAC on your clusters to assign and use the
    required roles. See Enabling fine-grained role-based access control
    for virtualization.

2.  You need access to the `openshift-cnv` namespace.

3.  You must have the `kubevirt.io:admin` role on both the source and
    target clusters.

4.  Install the latest supported version of OpenShift Container
    Platform. See the OpenShift Container Platform overview.

5.  Understand the required roles for migration operations and access to
    the Fleet Virtualization console. See the following table for more
    information about the required roles:

    - Role - Location - Purpose

    - acm-vm-fleet:admin - Hub cluster - Perform migration operations
      and access the Fleet Virtualization console.

    - acm-vm-extended:admin - Source and target clusters - Manage
      virtual machine extended and related resources.

    - acm-vm-cluster-migration:view - Target cluster - Perform mandatory
      cluster version and Red Hat OpenShift Virtualization operator
      readiness checks.
...
1.  Enable `cnv-mtv-integrations` component for the integration of Red
    ...
    1.  From the `components` specification, set the `enabled` parameter
        to `true` for the `cnv-mtv-integrations` component.
...
7.  **Optional:** If you want to further configure your environment in
    the managed cluster, edit the `HyperConverged` resource.
