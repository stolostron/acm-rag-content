...
uses in Red Hat Advanced Cluster Management for Kubernetes 2.16:
...
2.  To decrease the size of the persistent volumes remove the pods using
    the persistent volumes, delete the persistent volume and re-create
...
3.  Specify annotations for the observability service accounts that
    ...
    observability on Red Hat Red Hat OpenShift Service on AWS (classic)
    (ROSA) cluster can be configured to work with AWS STS tokens in the
    *Set environment* step. See Red Hat OpenShift Service on AWS (ROSA)
    for more details, along with ROSA with STS explained for an in-depth
...
- For the user workload for Prometheus, add a label to alerting rules in
  ...
  them. For help, see Enabling alert routing for user-defined projects
  in the OpenShift Container Platform documentation.
...
## *RightSizingRecommendation* guides
...
After you enable the observability service, the
`RightSizingRecommendation` feature is enabled by default for namespaces
and virtual machines. View the following descriptions of your
...
- After you enable the `RightSizingRecommendation` feature, it might
  take about 30 minutes for data to appear in the dashboard. The metric
  collection and aggregation with Thanos causes the delay.

### Optimizing workloads by using *RightSizingRecommendation* for namespaces

Implement `RightSizingRecommendation` to view a comparison of real-time
CPU and memory usage with the resource configuration that you set. After
you enable the observability service, the `RightSizingRecommendation`
feature for namespaces is enabled by default. You can disable the
feature by editing the `MultiClusterObservability` custom resource on
the hub cluster.
...
#### Disabling *RightSizingRecommendation* for namespaces

Complete the following steps to disable `RightSizingRecommendation` for
namespaces:
...
3.  To enable `RightSizingRecommendation` for namespaces, set the
    `namespaceRightSizingRecommendation.enabled:` to `true` .
...
### Configuring *RightSizingRecommendation* for namespaces
...
- You have enabled the `RightSizingRecommendation` specification. For
  ...
  RightSizingRecommendation guides for namespaces.
...
### Disabling *RightSizingRecommendation* for Virtualization workloads

Use `RightSizingRecommendation` for Virtualization to help you identify
over-provisioned or under-utilized virtual machine resources across
managed clusters. After you enable the observability service, the
`RightSizingRecommendation` feature is enabled by default. You can
disable the feature by editing the `MultiClusterObservability` custom
...
Complete the following steps to disable right-sizing for your virtual
machines:
...
3.  To enable `RightSizingRecommendation` for your virtual machines, set
    the `virtualizationRightSizingRecommendation.enabled:` parameter to
    `true`.
...
### Configuring *RightSizingRecommendation* for Virtualization
