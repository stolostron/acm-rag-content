...
Red Hat Advanced Cluster Management supports most of the template
functions from the `sprig` open source project, except for `env` and
`expandenv`. View the following table:
...
For more information, see Enabling etcd encryption in the OpenShift
...
input policy. To generate a Static Analysis Results Interchange Format
(SARIF) report of the linter results, specify a file path with the
`--sarif` argument.
