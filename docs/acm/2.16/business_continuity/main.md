...
- From your Red Hat OpenShift Container Platform cluster, install the
  Red Hat Advanced Cluster Management operator version 2.16. The
...
policy. Use the policy to view information about issues with your backup
...
The `backup-restore-enabled` policy validates the Red Hat Advanced
Cluster Management hub cluster backup and restore configuration and
applies to local hub clusters, global hub clusters, and managed clusters
where the Red Hat Advanced Cluster Management hub cluster is installed.
The policy includes a set of templates that check for the following
constraints:

- **Cluster backup component validation**

  - `acm-cluster-backup-enabled` template checks if the `cluster-backup`
    component is enabled in the `MultiClusterHub` resource. The template
    validates that the backup functionality has the correct
    configuration for the `MultiClusterHub` resource. If the
    `cluster-backup` component is disabled with the `enabled: false`
    flag in the `spec.overrides.components` field of the
    `MultiClusterHub` resource, the policy becomes `noncompliant` and
    displays instructions for enablement.
...
- **Auto import secret validation**
  ...
  - The `auto-import-account-secret` template checks whether a
    ...
    backup controller regularly scans for imported managed clusters.
    When a managed cluster is discovered, the backup controller creates
    the `ManagedServiceAccount` resource in the managed cluster
    namespace. This process starts the token creation on the managed
    cluster. If the managed cluster is not accessible at the time of
    this operation, the `ManagedServiceAccount` is unable to create the
    token. For example, if the managed cluster is hibernating, it is
    unable to create the token. If you then start a hub cluster backup,
    the backup lacks a token for automatically importing the managed
    cluster.
...
- **Auto import backup label validation**
  ...
  - The `auto-import-backup-label` template verifies the existence of a
    ...
    the secret. This label is required for including the
...
patterns. You must configure observability to ensure that your data
stays safe and keeps its continuity during the hub cluster migration or
backup.
